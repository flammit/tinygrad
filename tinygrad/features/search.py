...
def _time_program(variables:List[Variable], outcount:int, rdev:Compiled, lib:bytes, global_size, local_size, var_vals, rawbufs,
  ...
  if BEAM_LOG: print(f"_time_program: before car")
  ...
  tms = []
  for _ in range(cnt):
    if BEAM_LOG: print(f"_time_program: before rand")
    if clear_l2:
      with Context(DEBUG=0, BEAM=0, CACHECOLLECTING=0): Tensor.ones(1024,1024).contiguous().realize()
    if BEAM_LOG: print(f"_time_program: before car")
    try: tms.append(cast(float, car(rawbufs, var_vals, wait=True, do_update_stats=False))*factor)
    except BaseException as e:
      print(f"_time_program error {e}")
      return [math.inf] * cnt
    if BEAM_LOG: print(f"_time_program: after car")
...
def _try_compile_linearized_w_idx(x, compiler:Compiler):
  try: return (x[0], _compile_linearizer(compiler, x[1], "test"))
    ...
    print(f"search._compile_linearizer compile exception: {e}")
    print(f"ast={x[1].ast}")
    print(f"applied_opts={x[1].applied_opts}")
    print(x[1].colored_shape())
...
def beam_search(lin:Linearizer, rawbufs, amt:int, allow_test_size=True) -> Linearizer:
  ...
  if beam_pool is None and getenv("PARALLEL", default_parallel):
    ...
    beam_pool = ctx.Pool(multiprocessing.cpu_count(), _init_worker, (), None)
    ...
    while not exiting:
      ...
      for i,proc in (map(_compile_fn, enumerate(acted_lins)) if beam_pool is None else beam_pool.imap_unordered(_compile_fn, enumerate(acted_lins))):
        ...
        try: tms = _time_program(vars, outcount, dev, lib, global_size, local_size, var_vals, rawbufs, early_stop=beam[0][1]*3 if len(beam) else 1.0)
          ...
          print(f"beam_search: failed to _time_program: {e}")
          print(acted_lins[i].ast)
          print(acted_lins[i].applied_opts)
          print(acted_lins[i].colored_shape())
          continue
        timed_lins.append((acted_lins[i], min(tms)))
        if BEAM_LOG: print(f"{time.perf_counter() - st:7.2f}s: {timed_lins[-1][1]*1e6:12.2f} us       {len(timed_lins):4d}/{len(acted_lins):4d}         {timed_lins[-1][0].colored_shape()}")  # noqa: E501
  ...
  if beam_pool is not None and False:
    print(f"beam_pool before terminate")
    beam_pool.terminate()
    print(f"beam_pool before join")
    beam_pool.join()
    print(f"beam_pool after join")
...
def time_linearizer(lin:Linearizer, rawbufs:List[Buffer], allow_test_size=True, max_global_size=65536, cnt=3, disable_cache=False, clear_l2=False) -> float:  # noqa: E501
  ...
  var_vals = {k:(k.max+k.min)//2 for k in lin.ast[0].vars()}
  if BEAM_LOG: print(f"time_linearizer._compile_linearizer")
  lib, global_size, local_size, vars, outcount = _compile_linearizer(dev.compiler, lin)
  if BEAM_LOG: print(f"time_linearizer._time_program")
  ...
  if BEAM_LOG: print(f"time_linearizer._time_program: {min(tms)}")
